...
async def render_markdown(session: AsyncSession) -> str:
    ...
    buf = StringIO()
    w = buf.write
    w(f"# Taiwan Stock AI — Daily Research Report\n\n")
    ...
    w(f"## 強勢族群 (Sector Rotation)\n\n")
    w(f"| # | Sector | N | 5D | 20D | Leaders |\n")
    w(f"|---|---|---|---|---|---|\n")
    ...
    w(f"## ✅ Edge-Validated 訊號\n\n")
    if not brief["top_signals"]["validated"]:
        w(f"_今日無 edge-validated 訊號。研究候選請見下方。_\n\n")
    else:
        w(f"| SYM | SETUP | ENTRY | SL | TP1 | RR | CONF | WIN% | EXP_R | N | STATUS |\n")
        w(f"|---|---|---|---|---|---|---|---|---|---|---|\n")
